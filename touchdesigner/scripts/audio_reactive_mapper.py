...
print(
    "[LOAD-MARKER-V3] audio_reactive_mapper.py module loaded at absTime.frame="
    + str(absTime.frame)
)
...
#: Frame number this mapper last did its work on. See the guard at the top of
#: onValueChange -- this single variable is the graphics-freeze fix.
...
def _band_energy(spectrum_chop, lo, hi):
    ...
    2026-07-31: the sum was a Python `for i in range(lo, hi+1): total += abs(chan[i])`.
    Each `chan[i]` is a separate call across TD's Python/C++ boundary, and the
    MID band alone spans ~300 indices, so one invocation cost ~500 boundary
    crossings. `chan.vals` fetches the whole channel in ONE crossing and the sum
    then runs at C speed inside Python. Same arithmetic, roughly two orders of
    magnitude less overhead.
    """
    ...
    try:
        vals = chan.vals  # ONE boundary crossing for the whole channel
        total = 0.0
        for v in vals[lo : hi + 1]:
            total += v if v >= 0.0 else -v
    except Exception:
        # Older TD builds without .vals -- fall back to the per-index read.
        total = 0.0
...
def onValueChange(channel, sampleIndex, val, prev):
    ...
    ========================================================================
    2026-07-31 -- THE GRAPHICS-FREEZE FIX. Read this before removing the
    guard below.

    Symptom: graphics render fine in silence and FREEZE when music plays.
    That differential is the whole diagnosis. It is not GPU load, not the
    tracker, and not general CPU contention from OBS/BUTT/Serato -- none of
    those care whether audio is playing.

    A CHOP Execute DAT fires onValueChange **once per channel per sample
    whose value changed**, every frame. This DAT watches `audio_spectrum`,
    an FFT with hundreds of bins. In silence the bins are static, so almost
    nothing fires and the cost is invisible. Under music EVERY bin changes
    EVERY frame, so this function was being called once per bin -- hundreds
    of times per frame.

    And each call did the full job: two `op()` lookups by string, then three
    `_band_energy()` scans over the spectrum (~500 Python/C++ boundary
    crossings before the fix above), then a series of parameter writes. Call
    that ~500 crossings x ~300 bins = ~150,000 boundary crossings per frame,
    all on TD's cook thread holding the GIL. At 60fps that is millions per
    second. TD cannot cook through it, so the graphics stop -- exactly when
    the music starts.

    The work only ever NEEDED to happen once per frame: every one of those
    calls computed the same three band energies from the same spectrum and
    wrote the same parameters. The guard collapses N calls per frame into 1
    and throws nothing away.

    Keep this guard. If band energies ever need finer-than-frame resolution,
    the answer is a Script CHOP or native Analyze/Math CHOPs -- not removing
    this.
    ========================================================================
    """
    ...
    global _last_processed_frame

    # ---- once per frame, not once per FFT bin ----
    try:
        frame = absTime.frame
    except Exception:
        frame = -1
    if frame == _last_processed_frame and frame != -1:
        return
