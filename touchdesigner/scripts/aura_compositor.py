...
print('[syncfile-probe-v12] aura_compositor.py loaded — direct spectrum audio path active')
# ====================================================
...
# ---- [AUDIO-FIX-2026-04-26] Force BlackHole 2ch on every file load ----
try:
    ...
    if _fix_ai is not None:
        ...
        _fix_bh = next((n for n in _fix_names if 'blackhole' in (n or '').lower()), None)
        if _fix_bh:
            _fix_ai.par.device = _fix_bh
            _fix_ai.cook(force=True)
            print('[AUDIO-FIX] device forced to', _fix_bh, '| active:', _fix_ai.par.active.val)
        else:
            print('[AUDIO-FIX] BlackHole not in devices:', _fix_names)
    else:
...
def _force_reload_mask_reader_once():
    ...
    try:
        ...
        if not src_path:
            # Known-good fallback
            src_path = ('/Users/thomasadair/projects/touchdesigner-dj-suite'
        ...
        print('[mask-reload] OK. body_mask_top res=' +
              str(top.width) + 'x' + str(top.height) +
...
def _diag_render_chain_once():
    ...
    try:
        ...
        chain = [
            ...
            'final_composite',
            'output',
            'syphonOut1', 'syphonspoutOut1', 'syphonout1',
            'out1',
        ]
...
def _bootstrap_audio_chain_once():
    ...
    try:
        ...
        chosen = (
            ...
            or _find('built-in microphone', 'internal microphone', 'macbook', 'built-in')
        ...
        if chosen:
            ...
            print('[bootstrap] WARN no priority match, leaving device as ' + str(ai.par.device.val))
        ...
        print('[bootstrap] FINAL: driver=' + str(ai.par.driver.val) +
              ' | device=' + str(ai.par.device.val) +
...
def onFrameStart(frame):
    ...
    if asp is not None and asp.numSamples > 0:
        ...
        _smooth_bass = _smooth_bass * BASS_SMOOTH_COEF + bass_raw * (1.0 - BASS_SMOOTH_COEF)
        _smooth_mid  = _smooth_mid  * MID_SMOOTH_COEF  + mid_raw  * (1.0 - MID_SMOOTH_COEF)
        _smooth_high = _smooth_high * HIGH_SMOOTH_COEF + high_raw * (1.0 - HIGH_SMOOTH_COEF)
        ...
        if bass_delta > ONSET_THRESHOLD and (current_frame - _last_onset_frame) > cooldown_frames:
            _burst_decay = 1.0
    ...
    if frame % 120 == 0:
        print('[audio-direct] bass=' + str(round(b, 6)) +
              ' mid=' + str(round(m, 6)) +
              ' high=' + str(round(h, 6)) +
              ' | flame=' + str(round(flame_intensity, 4)) +
              ' sparkle=' + str(round(sparkle, 4)) +
              ' burst=' + str(round(burst_decay, 4)))
