...
# v2 chain (matches build_network_v2.py). v1 names removed:
#   noise_embers, motion_energy_top, audio_energy_top, flame_layers,
#   burst_flash, camera_in, output
#   — camera_in is INTENTIONALLY absent in v2 (OBS owns the camera);
#     "output" is the Null TOP "final_composite".
chain = [
    "body_mask_top",
    "edge_detect",
    "edge_blur",
...
if root is not None:
    ...
    for child in root.children:
        ...
        if "syphon" in tname or "syphon" in nm or "spout" in tname or "spout" in nm:
            ...
            print(
                "    res="
                + str(getattr(child, "width", "?"))
                + "x"
                + str(getattr(child, "height", "?"))
            )
            if errs:
                print("    ERR=" + errs[:200])
            if warns:
